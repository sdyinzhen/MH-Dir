...
# modified by David Yin for P-field simulation, yinzhen@stanford.edu, Aug 03, 2020
import pandas as pd
import os
import numpy as np
import matplotlib.pyplot as plt                          
import random as rand
image_type = 'tif'; dpi = 150
...
def locmap_st(df,xcol,ycol,vcol,xmin,xmax,ymin,ymax,vmin,vmax,title,xlabel,ylabel,vlabel,cmap): 
    model = np.zeros((int(ymax), int(xmax)))
    model[:,:] = np.nan
    plt.imshow(model)
    im = plt.scatter(df[xcol],df[ycol],s=None, \
                     c=df[vcol], marker=None, cmap=cmap, norm=None, vmin=vmin, vmax=vmax, \
